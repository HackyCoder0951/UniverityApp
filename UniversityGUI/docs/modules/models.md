...
public class Section {
    ...
    public void dropStudent(Student student) {
        if (enrolledStudents.remove(student)) {
            enrolledCount--;
            if (enrolledCount < capacity && "CLOSED".equals(status)) {
                // If a spot freed up, reopen the section
                status = "OPEN";
            }
        }
    }
...
There are additional model classes in this module, including `Instructor`, `Department`, `Classroom`, `TimeSlot`, `Role`, `Marks`, `Result`, and various DAO-related DTOs. Each follows the same conventions: private fields, constructors, getters/setters with validation, business helper methods, and sensible `toString`/`equals`/`hashCode` implementations. Refer to the `src/main/java/com/university/app/model` package for the full source.
